...
class PersistentPluginSelectView(miru.View):
    ...
    async def on_plugin_select(self, ctx: miru.ViewContext) -> None:
        ...
        try:
            ...
            # Debug logging
            logger.debug(f"Looking for help plugin instance...")
            logger.debug(f"Found help plugin: {help_plugin_instance}")

            if not help_plugin_instance:
                await ctx.respond("Help system temporarily unavailable. Please try using the help command again.", flags=hikari.MessageFlag.EPHEMERAL)
                return
            ...
            if selected_value == "loading":
                await ctx.respond("Please wait for the help system to fully load.", flags=hikari.MessageFlag.EPHEMERAL)
                return
            ...
            await ctx.respond("An error occurred while processing your request.", flags=hikari.MessageFlag.EPHEMERAL)
...
class HelpPlugin(BasePlugin):
    ...
    async def _get_plugin_commands_embed(self, plugin_name: str) -> Optional[hikari.Embed]:
        ...
        if prefix_commands:
            ...
            embed.add_field(
                "📝 Commands",
                "No commands available in this plugin.",
                inline=False
            )
    ...
    async def _get_command_info(self, command_name: str) -> Optional[dict]:
