...
class DiscordAuth:
    ...
    async def handle_callback(self, request: Request) -> dict[str, Any]:
        ...
        try:
            ...
            accessible_guilds = [guild for guild in user_guilds if str(guild["id"]) in bot_guild_ids]
    ...
    async def revoke_token(self, access_token: str):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            data = {"token": access_token, "client_id": settings.discord_client_id, "client_secret": settings.discord_client_secret}

            headers = {"Content-Type": "application/x-www-form-urlencoded"}

            async with session.post("https://discord.com/api/oauth2/token/revoke", data=data, headers=headers) as resp:
                if resp.status != 200:
                    raise HTTPException(status_code=resp.status, detail=f"Failed to revoke token: {await resp.text()}")
    ...
    def require_guild_access(self, request: Request, guild_id: str) -> bool:
        ...
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Access denied for this guild")
