...
def requires_permission(permission_node: str, error_message: Optional[str] = None) -> Callable:
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(ctx: lightbulb.Context, *args, **kwargs) -> Any:
            ...
            if bot and hasattr(bot, "permission_manager") and isinstance(ctx.member, hikari.Member):
                ...
                logger.info(f"Permission result: {ctx.author.username} {'HAS' if has_perm else 'DENIED'} permission '{permission_node}'")
                ...
                logger.warning(f"Permission check skipped: No permission manager or not a guild member")
...
def requires_guild_owner(error_message: Optional[str] = None) -> Callable:
    def decorator(func: Callable) -> Callable:
        @functools.wraps(func)
        async def wrapper(ctx: lightbulb.Context, *args, **kwargs) -> Any:
            if not ctx.guild_id:
